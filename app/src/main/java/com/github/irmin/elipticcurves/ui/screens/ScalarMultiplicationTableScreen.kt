...
import com.github.irmin.elipticcurves.presentation.viewmodel.ScalarMultiplicationTableViewModel
import com.github.irmin.elipticcurves.ui.components.TableCell

@OptIn(ExperimentalMaterial3Api::class)
@Composable
fun ScalarMultiplicationTableScreen(
    onBack: () -> Unit,
    modifier: Modifier = Modifier,
    viewModel: ScalarMultiplicationTableViewModel = viewModel()
) {
    ...
    Scaffold(
        topBar = {
            TopAppBar(
                title = { Text("Tabla de Multiplicación Escalar") },
                navigationIcon = {
            ...
            if (state.calculated) {
                Card(
                    ...
                    Column(modifier = Modifier.padding(8.dp)) {
                        ...
                        val generatorBg = MaterialTheme.colorScheme.tertiaryContainer.copy(alpha = 0.7f)
                                ...
                                state.rows.forEachIndexed { index, row ->
                                    ...
                                    Row(modifier = Modifier.background(rowBg)) {
                                        TableCell(
                                            ...
                                            borderColor = borderColor,
                                            width = 140,
                                            fontWeight = if (row.isGenerator) FontWeight.Bold else FontWeight.Normal
                                        )
                                        row.multiples.forEach { value ->
                                            TableCell(
                                                text = value,
                                                bg = Color.Transparent,
                                                borderColor = borderColor,
